 ...
 *  5. In production, /auth/* calls are routed through this same domain's
 *     /api/auth/* rewrite (see next.config.js) so the refresh cookie is
 *     first-party -- iOS blocks third-party cookies, which broke "stay
 *     logged in" on iPhone before this. Everything else talks straight to
 *     the backend, since only auth actually depends on that cookie --
 *     routing every request through the extra hop was a real, needless
 *     slowdown on every feature in the app.
 *
...
const DIRECT_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api';
const AUTH_PROXY_BASE_URL = '/api'; // same-origin, only used for /auth/* in production
...
// -- Request interceptor: attach Bearer token, route auth calls through the proxy --
api.interceptors.request.use(
  (config) => {
    ...
    // Only /auth/* needs the same-origin cookie trick; everything else goes
    // straight to the backend for speed.
    if (process.env.NODE_ENV === 'production' && config.url && config.url.indexOf('/auth/') === 0) {
      config.baseURL = AUTH_PROXY_BASE_URL;
    }
    return config;
  },
