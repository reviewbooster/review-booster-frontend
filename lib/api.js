...
// In production, calls go through this same domain's /api/* rewrite (see
// next.config.js) so the login cookie is first-party -- iOS blocks
// third-party cookies, which broke "stay logged in" on iPhone before this.
// In development there's no rewrite, so we talk to the backend directly.
