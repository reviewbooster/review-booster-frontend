 ...
 * removed from here ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Team now has its own dedicated page.
 */
...
// Strips the fixed referral-info + link block from the end of the Thank + Refer
// template for display in the editable textarea (mirrors stripLinkToken above,
// but this field has extra fixed content after the editable middle text).
...
function SettingsPage() {
  ...
  var handleSaveTemplates = async function() {
    ...
    try {
      var res = await api.patch('/business/my-settings', { message_templates: templates });
      setBusiness(res.data.data);
